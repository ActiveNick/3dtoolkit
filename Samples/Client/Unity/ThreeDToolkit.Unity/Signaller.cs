...
namespace ThreeDToolkit
{
    public class Signaller : ISignaller
    {
            ...
            var req = new HttpRequest(uri)
                ...
                .Done((HttpWebResponse res) =>
                {
                    if (res.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        var pragmaId = int.Parse(res.GetResponseHeader("Pragma"));
        ...
        private void StartHangingGet()
        {
            ...
            if (this.hangingGetThread != null && this.hangingGetThread.IsAlive)
            {
                this.hangingGetThread.Abort();
                this.hangingGetThread.Join();
            }

            // create it
            this.hangingGetThread = new Thread(() =>
            {
